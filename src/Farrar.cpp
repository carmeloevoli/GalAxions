...
std::vector<double> FarrarField::GetB(const double& x_, const double& y_, const double& z_) {
	std::vector<double> Bret(3, 0);

	const double r = std::sqrt(x_ * x_ + y_ * y_);
	if (r > 20 || std::sqrt(r * r + z_ * z_) < .1)
		return Bret;
	...
	if (r > 3.0) {
		if (r < 5.0) {
			...
			const double tanfactor = 1.0 / std::tan(PiOver2 - p);
			double rxneg = r * std::exp(-(phi - M_PI) * tanfactor);
			if (rxneg > rj[7])
				rxneg = r * std::exp(-(phi + M_PI) * tanfactor);
			if (rxneg > rj[7])
				rxneg = r * std::exp(-(phi + 3.0 * M_PI) * tanfactor);

			for (int loopcounter = 7; loopcounter >= 0; loopcounter--) {
				if (rxneg < rj[loopcounter])
					Bdisk = bj[loopcounter];
			}
