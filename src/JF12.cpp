...
std::vector<double> JF12Field::getRegularField(const double &x, const double &y,
...
double JF12Field::getTurbulentStrength(const double &x, const double &y,
		const double &z) const {
	double r = sqrt(x * x + y * y); // in-plane radius
	if (r > 20 * kpc)
		return 0;
	...
	double bHalo = bHaloTurb * exp(-r / rHaloTurb)
			* exp(-0.5 * pow(z / zHaloTurb, 2));

	// modulate turbulent field
	return sqrt(pow(bDisk, 2) + pow(bHalo, 2));
}

std::vector<double> JF12Field::getTurbulentField(const double &x,
...
std::vector<double> JF12Field::GetB(const double &x, const double &y,
		const double &z) {
	std::vector<double> b(3, 0);
	if (useTurbulent) {
