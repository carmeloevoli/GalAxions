...
//  printf("m_6=%d\n\n",m_6);
    ...
    *WW = UU;
    ...
    if ((UU - Rlb) > (mc * t6.wlb1) || fabs(zz) > (mc * t6.hlb1)) {
        ...
        nelb1 = glb1 * pow(2 / (exp(-VVV / t6.detlb1) + exp(VVV / t6.detlb1)), 2) * t6.nlb1
                * pow(2 / (exp(g4) + exp(-g4)), 2) * g5;
    }
    ...
    if (nelb1 == 0 && nelb2 == 0) {
        *ne6 = 0;
        ...
        nelb2 = glb2 * pow(2 / (exp(-WWW / t6.detlb2) + exp(WWW / t6.detlb2)), 2) * t6.nlb2
                * pow(2 / (exp(g6) + exp(-g6)), 2) * g7;
    }
